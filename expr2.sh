...
for epoch in "${epochs[@]}"; do
  ...
  LD_PRELOAD=/path/to/hvac_ver1/build/src/libhvac_client.so horovodrun --start-timeout 120 --gloo -np 1024 --min-np 1 --hostfile hostfile_horovod python train2.py -d --batch-size=16 --prefetch=2 --seed=0 --kill-times 1 --epochs-to-kill $epoch | tee ${HVAC_LOG_DIR}/output_ver1_epoch${epoch}.log
  ...
  LD_PRELOAD=/path/to/hvac_ver2/build/src/libhvac_client.so horovodrun --start-timeout 120 --gloo -np 1024 --min-np 1 --hostfile hostfile_horovod python train2.py -d --batch-size=16 --prefetch=2 --seed=0 --kill-times 1 --epochs-to-kill $epoch | tee ${HVAC_LOG_DIR}/output_ver2_epoch${epoch}.log
  sh kill.sh #kill hvac_server
  ### COPY LOGDIR INTO THE SAFE PLACE && EMPTY THE BB ###
done
