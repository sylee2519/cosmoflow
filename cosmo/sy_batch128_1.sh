...
sh host5.sh ${NumNode} # Create a hostfile
...
# Version 1
export HVAC_LOG_DIR="/lustre/orion/proj-shared/stf008/hvac/sylee/cosmoflow/logdir/expr1/v128_1" # Must be inside BB path to minimize interference
#srun --nodes=${NumNode} --ntasks=${NumNode} --gres=gpu:0 --ntasks-per-node=1 --gpus-per-task=0 --nodelist=$NODELIST mkdir -p ${HVAC_LOG_DIR} # Each node create the LOGDIR in BB
srun --kill-on-bad-exit=0 --export=ALL,HVAC_LOG_DIR=${HVAC_LOG_DIR},HVAC_SERVER_COUNT=${NumNode},MY_JOBID=5 --nodes=${NumNode} --ntasks=${NumNode} --gres=gpu:0 --ntasks-per-node=1 --gpus-per-task=0 --nodelist=$NODELIST ${HVAC_V1_ServerPath} 1 &
# This has to be run with horovodrun instead of srun (gloo - horovod elastic run for the cosmoflow to support fault tolerance)
LD_PRELOAD=${HVAC_V1_ClientPath}  horovodrun --start-timeout 4600 --gloo -np 128 --min-np 1 --hostfile ${hostfile} env HVAC_LOG_DIR=${HVAC_LOG_DIR} MY_JOBID=5 HVAC_SERVER_COUNT=${NumNode} python train5.py -d --batch-size=2 --prefetch=0 --seed=0 --kill-times 1 --epochs-to-kill 2 
