...
sh host1.sh ${NumNode} # Create a hostfile
...
# Version 1
export HVAC_LOG_DIR="/lustre/orion/proj-shared/stf008/hvac/sylee/cosmoflow/logdir/expr1/test" # Must be inside BB path to minimize interference
#srun --nodes=${NumNode} --ntasks=${NumNode} --gres=gpu:0 --ntasks-per-node=1 --gpus-per-task=0 --nodelist=$NODELIST mkdir -p ${HVAC_LOG_DIR} # Each node create the LOGDIR in BB
...
LD_PRELOAD=${HVAC_V1_ClientPath}  horovodrun --start-timeout 1200 --gloo -np 16 --min-np 1 --hostfile ${hostfile} env HVAC_LOG_DIR=${HVAC_LOG_DIR} MY_JOBID=1 HVAC_SERVER_COUNT=${NumNode} python train3.py -d --n-train=128 --n-valid=128 --batch-size=4 --prefetch=2 --seed=0 --kill-times 1 --epochs-to-kill 2 2>&1 | tee ./output_ver1.log
